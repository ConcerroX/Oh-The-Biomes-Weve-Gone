...
public class BWGWoodSet {
    ...
    protected BWGWoodSet(BlockSetType blockSetType, MapColor mapColor, LogStem logstem, @Nullable Supplier<TreeGrower> saplingGrower, boolean leaves, boolean glowLeaves, @Nullable TagKey<Block> saplingPlantAbleOn) {
        ...
        this.wallSign = BWGWood.register(name + "_wall_sign", properties -> new BWGWallSignBlock(properties, woodType), BlockBehaviour.Properties.of().mapColor(this.logstem.get().defaultMapColor()).forceSolidOn().instrument(NoteBlockInstrument.BASS).noCollission().strength(1.0F).overrideLootTable(sign.get().getLootTable()).ignitedByLava());
        ...
        this.wallHangingSign = BWGWood.register(name + "_wall_hanging_sign", properties -> new BWGWallHangingSignBlock(properties, woodType), BlockBehaviour.Properties.of().mapColor(mapColor).forceSolidOn().instrument(NoteBlockInstrument.BASS).noCollission().strength(1.0F).overrideLootTable(hangingSign.get().getLootTable()).ignitedByLava());
