...
public class BWGQuickSand extends ColoredFallingBlock {
	...
	public void entityInside(@NotNull BlockState state, @NotNull Level level, @NotNull BlockPos pos, @NotNull Entity entity) {
		if (entity instanceof LivingEntity) {
			...
			} else if (level.getBlockState(new BlockPos(entity.getBlockX(), (int) entity.getEyeY(), entity.getBlockZ())).getBlock() instanceof BWGQuickSand)
				entity.hurtServer((ServerLevel) level, new DamageSource(level.registryAccess().lookupOrThrow(Registries.DAMAGE_TYPE).getOrThrow(BWGDamageTypes.IN_QUICKSAND)), 0.5F);
		}
